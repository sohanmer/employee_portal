...
<div style="height:4em">
    <div class="container">
        ...
        <div class=" font-weight-bold ml-4 d-inline-block">
            <h4><a href="/home" style="text-decoration:none"><strong>HOME </strong></a><strong>/ Search Result</strong></h4>
        </div>
...
<div class="container" id="grid" style="display:none">
    @if(!Auth::guest()) @for($i=0; $i
    <count($searchResult); $i++ ) @foreach($allRecord as $file) @if($file[ 'file_name']==$searchResult[$i]) <div class="well col-md-3 mt-5 mr-5 ml-5 mb-2">
        <div class="row" style="height:25px">
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <center>
                                ...
                                'result')}} {{Form::hidden('id', $file['id'])}} {{Form::text('rename', 'New Name')}} {{Form::submit('Submit',
            ...
            <div class="col-md-8 col-sm-8">
                <div class="dropdown">
                    ...
                    <h5><a href="{{ URL::route('files.show', $file->id) }}">{{$file['file_name']}}</a></span>
                    </h5>
                        ...
                        <a class="dropdown-item">
                                                    @if(!Auth::guest())
                                                        ...
                                                        @if(Auth::user()->id == $file["owner_id"])
                                                            ...
                                                            <a class="mb-2 dropdown-item" data-toggle="modal" data-target="#rename{{$file->id}}"><i class="fas fa-edit  ml-4 mr-4"></i>Rename</a>                        @endif @endif
                        </a>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-8 col-sm-8">
                <div class="dropdown">
                    ...
                    <div class="dropdown-menu" aria-labelledby="file">
                        ...
                        <a class="dropdown-item">
                                                            @if(!Auth::guest())
                                                                @hasrole('admin')
                                                                    ...
                                                                    {!!Form::close()!!}
                                                                    <a class="mb-2 dropdown-item" data-toggle="modal" data-target="#rename{{$file->id}}"><i class="fas fa-edit  ml-4 mr-4"></i>Rename</a>                        @else @if(Auth::user()->id == $file["owner_id"]) {!!Form::open(['action'=>['ActionController@destroy',
                        ...
                        border-0 bg-transparent mr-2 ml-3'] )}} Delete {!!Form::close()!!}
...
<div class="container" id="list" style="display:none">
    ...
    <table class="table">
        ...
        @for($i=0; $i
        <count($searchResult); $i++ ) @foreach($allRecord as $file) @if($file[ 'file_name']==$searchResult[$i]) <div class="modal fade"
            id="renamelist{{$file->id}}" tabindex="-1" role="dialog" aria-labelledby="rename" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="renamelist">Rename{{$file['file_name']}}</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                <span aria-hidden="true">&times;</span>
                                            </button>
                    </div>
                    <div class="modal-body">
                        <center>
                            {!! Form::open(['action' => 'ActionController@update', 'method'=>'POST','enctype'=>'multipart/form-data']) !!} {{Form::hidden('id',
                            $file['id'])}} {{Form::text('rename', 'New Name')}} {{Form::submit('Submit', ['class'=> 'btn
    ...
    <tr>
        ...
        <td>
            <ul class="list-inline">
                ...
                @endif @if(!Auth::guest()) @if(Auth::user()->id == $file["owner_id"])
                <li>
                    {!!Form::open(['action'=>['ActionController@destroy', $file['id']], 'method'=>'POST'])!!} {{Form::hidden('_method','DELETE')}}
                    {{Form::button('
                    <i class="fas fa-trash-alt"></i>', ['type' => 'submit', 'class' => ' text-danger border-0 border-0 bg-transparent']
                    )}} {!!Form::close()!!}
                </li>
                <li data-toggle="modal" data-target="#renamelist{{$file->id}}"><i class="fas fa-edit "></i></li>

    ...
    <div>
        <div class="container">
            <div class="font-weight-bold ml-4">
                <h3 class="mt-5"><strong>Home</strong></h3>
            </div>
        <div class="mt-5">                
            <button class="btn btn-primary pull-right align-middle ml-3" type="button" id="dropdownMenuButton" data-toggle="dropdown">
                Add
            </button>
            ...
            <div class="dropdown pull-right ml-3">
                ...
                <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
                    ...
                    <a href="/home?type=" class="dropdown-item">All</a>
                    @foreach( array_keys(Config::get('constants.filters')) as $filterType)
                        <a href="/home?type={{$filterType}}"  class="dropdown-item">{{$filterType}}</a>
                    @endforeach
                </div>  
            </div>
        ...
        @if(count($allFile)>0) 
            @foreach($allFile as $file) 
                @if($file['parent_directory']==null)
                    <div class="well col-md-3 col-sm-12 mr-5 ml-3 mb-5">
                        <div class="row" style="height:25px">
                            ...
                            <div class="modal fade" id="folderinfo{{$file->id}}" tabindex="-1" role="dialog" aria-labelledby="folderinfo" aria-hidden="true">
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title font-weight-bold" id="folderinfo">Item Details</h5>
                                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                <span aria-hidden="true">&times;</span>
                                            </button>
                                        </div>
                                        <div class="modal-body">
                                            <h6><span class="font-weight-bold">Name:</span> {{$file["file_name"]}}</h6>
                                            <h6><span class="font-weight-bold">Owned By:</span> {{$file["owner_name"]}}</h6>
                                            <h6><span class="font-weight-bold">Created On:</span> {{date('d-m-Y',strtotime($file["created_at"]))}}</h6>
                                        </div>
                                    </div>
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                            ...
                            @if($file['cover_image'] == null and $file['parent_directory']==null)
                                <div class="col-md-8 col-sm-8">
                                    <div class="dropdown">
                                        ...
                                        <h5><a href="{{route('files.show',$file->id)}}?type={{session()->get('filter')}}"  >{{$file['file_name']}}</a></h5>
                                            ...
                                            <a class="mb-2 dropdown-item" data-toggle="modal" data-target="#folderinfo{{$file->id}}"><i class="far fa-question-circle mr-3 ml-4"></i>  Details</a>
                                                <a class="dropdown-item">
                                                    ...
                                                    @else 
                                                        ...
                                                        {!!Form::open(['action'=>['ActionController@destroy',$file['id']], 'method'=>'POST'])!!} 
                                                            {{Form::hidden('_method','DELETE')}} 
                                                            {{Form::button('<i class="fas fa-trash-alt mr-4"></i>Delete', ['type' => 'submit', 'class' => ' text-danger border-0 align-top bg-transparent border-0 dropdown-item'] )}} 
                                                        {!!Form::close()!!}
                                                        <a class="mb-2 dropdown-item" data-toggle="modal" data-target="#rename{{$file->id}}"><i class="fas fa-edit  ml-4 mr-3"></i>Rename</a>              
                                                        @endif 
                                                        @endhasrole 
                                                    @endif
                                                </a>
                                            ...
                                            @if(!Auth::guest()) 
                                                @hasrole('admin')
                                                    <a class="dropdown-item">
                                                        {!!Form::open(['action'=>['ActionController@destroy', $file['id']], 'method'=>'POST'])!!}
                                                            {{Form::hidden('_method','DELETE')}}
                                                            {{Form::button('<i class="fas fa-trash-alt"></i>', ['type' => 'submit', 'class' => ' text-danger border-0 align-top border-0 bg-transparent mr-5 ml-3']  )}} Delete
                                                        {!!Form::close()!!}
                                                    </a>
                                                    <a class="mb-2 dropdown-item" data-toggle="modal" data-target="#rename{{$file->id}}"><i class="fas fa-edit  ml-4 mr-4"></i>Rename</a> 
                                            @else 
                    ...
                    @if($file['parent_directory']==null)
                        <tbody>
                            <tr>
                                ...
                                @else
                                    <h3>
                                        <th><a href="{{route('files.show',$file->id)}}?type={{session()->get('filter')}}"  >{{$file['file_name']}}</a></th>
                                    </h3>
                                @endif
                                ...
                                <td>
                                    <ul class="list-inline">
                                        @if($file['type']!=null)
                                            <li><a href="/download?id={{$file['id']}}" class="dropdown-item"><i class="fas fa-download "></i></a></li>
                                        @else
